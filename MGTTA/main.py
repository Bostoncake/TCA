...
# TCA baseline
from models.tca import apply_patch_TCAPromptViT
# FLOPs analysis
...
def get_args():
    ...
    # TCA baseline (Token Condensation as Adaptation, ICCV 2025)
    parser.add_argument('--apply_tca', default=False, action='store_true', help='TCA token condensation baseline (ICCV 2025)')
    parser.add_argument('--tca_r', default=None, type=int_or_str, help='TCA: number of tier-3 tokens removed per pruning layer (int or underscore-separated schedule)')
    parser.add_argument('--tca_prune_layer', default="3,6,9", type=str, help='TCA: comma-separated layer indices where token condensation is applied')
    parser.add_argument('--tca_num_coreset_centers', default=4, type=int, help='TCA: number of coreset cluster centers for tier-2 tokens')
...
if __name__ == '__main__':
    ...
    if args.algorithm == 'tent':
        ...
        if args.apply_tca and args.tca_r is not None:
            print("Apply TCA token condensation.")
            apply_patch_TCAPromptViT(net, tca_r=args.tca_r, prune_layer=args.tca_prune_layer, num_coreset_centers=args.tca_num_coreset_centers)
        ...
        _is_token_pruning_baseline = args.apply_evit or args.apply_tofu or args.apply_tca or args.as_baseline
        if not _is_token_pruning_baseline:
            # NOTE: SOTA chooses this route
