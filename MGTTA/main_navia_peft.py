...
import tta_library.foa_bp_peft as foa_bp
...
def get_args():
    ...
    parser.add_argument('--vpt', default=0, type=int) 
    parser.add_argument('--lora', default=0, type=int) 
    parser.add_argument('--peft_layer', default='0,1,2,3,4,5', type=str, help='Layers in which Token Dispatcher is applied before the FFN module, comma separated')
...
if __name__ == '__main__':
    ...
    if args.algorithm == 'tent':
        ...
        if args.vpt > 0:
            print("Apply VPT")
            net = PromptViT(net, 0).cuda()
            from models.tome_vpt import apply_patch_ToMePromptViT_VPT
            apply_patch_ToMePromptViT_VPT(net, args.tome_r, vpt_layer=args.peft_layer, num_prompts=args.vpt)
            net = foa_bp.configure_model(net).cuda()
            params, params_names = foa_bp.collect_params_peft(net, 0.005)
            ...
            print("Apply LoRA")
            net = PromptViT(net, 0).cuda()
            from models.tome_lora import apply_patch_ToMePromptViT_LoRA
            apply_patch_ToMePromptViT_LoRA(net, args.tome_r, lora_layers=args.peft_layer, lora_rank=args.lora, lora_targets=['q', 'k', 'v'])
            net = foa_bp.configure_model(net).cuda()
            params, params_names = foa_bp.collect_params_peft(net, 0.005)
            logger.info(f"FOA BP training params: {str(params_names)}")
            optimizer = torch.optim.SGD(params, momentum=0.9)
