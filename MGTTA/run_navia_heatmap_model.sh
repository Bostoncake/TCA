...
if [ "$GPU" -eq 2 ]; then
    ...
    lr_cls_ssf=5e-2
    load_model_dir="/home/xiongyizhe/CVPR2026/MGTTA/outputs/foa_bp_2025-11-16_12_base_ToMe_8_navia_heatmap_model/rendition-base-model.pth"
    ...
    lr_cls_ssf=3e-2
    load_model_dir="/home/xiongyizhe/CVPR2026/MGTTA/outputs/foa_bp_2025-11-16_16_large_ToMe_4_navia_heatmap_model/rendition-large-model.pth"
    heatmap_layer="0,11,22"
fi
algorithm="foa_bp"


# tag=$currenttime"_"$backbone"_ToMe_"$tome_r"_navia_heatmap_model"
...
#     --tag $tag --save_model > /home/xiongyizhe/CVPR2026/MGTTA/outputs/$algorithm"_"$tag/$corruption-running-log.txt

tag=$currenttime"_"$backbone"_ToMe_"$tome_r"_navia_heatmap_draw"

mkdir -p /home/xiongyizhe/CVPR2026/MGTTA/outputs/$algorithm"_"$tag/

# test_img_dir="/mnt/data1/xiongyizhe/dataset/imnet-ood/imagenet-a/n02077923/0.006014_submarine _ submarine_0.7892372.jpg"
...
python3 heatmap.py \
    ...
    --algorithm $algorithm \
    --apply_tome --tome_r $tome_r --apply_sep_lr_for_cls --lr_cls_token $lr_cls_token --apply_cls_ssf --cls_ssf_layer $cls_ssf_layer --apply_sep_lr_cls_ssf --lr_cls_ssf $lr_cls_ssf \
    --tag $tag --load_model_dir $load_model_dir --test_image_dir "$test_img_dir" --heatmap_layer $heatmap_layer # > /home/xiongyizhe/CVPR2026/MGTTA/outputs/$algorithm"_"$tag/$corruption-running-log.txt