...
class ToMePromptViTBlock(Block):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if r > 0:
            ...
            x, self._tome_info["size"] = merge_wavg(merge, x, self._tome_info["size"])
        if self.use_cls_ssf:
            # TODO: only shift, no scale
            cls_token = x[..., :1, :] + self.cls_ssf_bias
            # cls_token = x[..., :1, :] * self.cls_ssf_scale + self.cls_ssf_bias
            x = torch.cat([
                cls_token,
                x[..., 1:, :]
            ], dim=1)
...
def apply_patch_ToMePromptViT_clsSSF(
    model: PromptViT, tome_r: int, trace_source: bool = False, prop_attn: bool = True, ssf_layer: str = None
):
    """
    Applies ToMe to this transformer. Afterward, set r using model.r.
    ...
    the shelf. For trianing and for evaluating MAE models off the self set this to be False.
    """
    assert ssf_layer is not None
    ToMePromptViT = make_tome_class_ToMePromptViT(model.__class__)

    ssf_layer = [int(layer) for layer in ssf_layer.strip().split(",")]
    ...
    for layer_idx, block in enumerate(model.vit.blocks):
        if layer_idx in ssf_layer:
            block.use_cls_ssf = True
            # TODO: only scale, no shift
            block.cls_ssf_bias = init_ssf_scale_shift(model.vit.embed_dim)
            # block.cls_ssf_scale, block.cls_ssf_bias = init_ssf_scale_shift(model.vit.embed_dim)
        else:
            block.use_cls_ssf = False