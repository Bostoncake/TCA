...
class PYRA_ori(nn.Module):
    def __init__(self, dim_token=None, dim_channel=None, dim_token_init="sqrt", dim_channel_init="zero"):
        super().__init__()
        assert dim_token is not None or dim_channel is not None
        self.use_pyra_token = False
        self.use_pyra_channel = False
        if dim_token is not None:
            self.pyra_token = nn.Parameter(torch.zeros(1, dim_token))
            self.use_pyra_token = True
            assert dim_token_init in ("sqrt", "zero")
            if dim_token_init == "zero":
                nn.init.zeros_(self.pyra_token)
            elif dim_token_init == "sqrt":
                nn.init.kaiming_uniform_(self.pyra_token, a=math.sqrt(5))
        if dim_channel is not None:
            self.pyra_channel = nn.Parameter(torch.zeros(dim_channel, 1))
            self.use_pyra_channel = True
            assert dim_channel_init in ("sqrt", "zero")
            if dim_channel_init == "zero":
                nn.init.zeros_(self.pyra_channel)
            elif dim_channel_init == "sqrt":
                nn.init.kaiming_uniform_(self.pyra_channel, a=math.sqrt(5))

    def forward(self, x):
        if self.use_pyra_token:
            weight_channel = 2*torch.sigmoid(self.pyra_token @ x) - 1
        else:
            weight_channel = 1
        if self.use_pyra_channel:
            weight_token = 2*torch.sigmoid(x @ self.pyra_channel) - 1
        else:
            weight_token = 1
        x = x + (x * weight_channel) * weight_token
        return x
...
class PYRA(nn.Module):
    def __init__(self, dim_token=None, dim_channel=None, dim_token_init="sqrt", dim_channel_init="zero"):
        ...
        self.use_pyra_channel = False
        if dim_token is not None:
            self.pyra_token = nn.Parameter(torch.zeros(dim_token, 1))
            self.use_pyra_token = True
            assert dim_token_init in ("sqrt", "zero")
            if dim_token_init == "zero":
                nn.init.zeros_(self.pyra_token)
                ...
                nn.init.kaiming_uniform_(self.pyra_token, a=math.sqrt(5))
        if dim_channel is not None:
            self.pyra_channel = nn.Parameter(torch.zeros(1, dim_channel))
            self.use_pyra_channel = True
            assert dim_channel_init in ("sqrt", "zero")
            if dim_channel_init == "zero":
                nn.init.zeros_(self.pyra_channel)
    ...
    def forward(self, x):
        if self.use_pyra_token:
            weight_channel = 2*torch.sigmoid(self.pyra_token * x)
        else:
            weight_channel = 1
        if self.use_pyra_channel:
            weight_token = 2*torch.sigmoid(x * self.pyra_channel)
        else:
            weight_token = 1
        x = (x * weight_channel) * weight_token
        return x
...
def bipartite_soft_matching(
    ...
    def merge(x: torch.Tensor, mode="mean", use_pyra=False) -> torch.Tensor:
        src, dst = x[..., ::2, :], x[..., 1::2, :]
        n, t1, c = src.shape
        unm = src.gather(dim=-2, index=unm_idx.expand(n, t1 - r, c))
        src = src.gather(dim=-2, index=src_idx.expand(n, r, c))
        if use_pyra and pyra is not None:
            src = pyra(src)
...
def merge_wavg(
    ...
    x = merge(x * size, mode="sum", use_pyra=True)
    size = merge(size, mode="sum", use_pyra=False)
...
def apply_patch_PYRAPromptViT(
    ...
    r_list = parse_r(len(model.vit.blocks), model.r)
    for i, block in enumerate(model.vit.blocks):
        if r_list[i] > 0:
            print(f"block: {i}, pyra: {r_list[i]}")
            block.pyra = PYRA(dim_token=r_list[i], dim_channel=768, dim_channel_init="zero", dim_token_init="zero")
        else:
            block.pyra = None