...
if __name__ == '__main__':
    ...
    if args.algorithm == 'tent':
        ...
        _, train_loader = obtain_train_loader(args)
        adapt_model.obtain_origin_stat(train_loader, count_flops=True)
    ...
    for corrupt in corruptions:
        ...
        training_para_nums = 0
        for name,param in adapt_model.named_parameters():
            if param.requires_grad:
                training_para_nums += param.numel()
        ...
        if args.algorithm == 'train_mgg':
            ...
            flops, timing, memory = validate(val_loader, adapt_model, args)
        
        print("Total GFLOPs per image:", (flops.total() / args.batch_size)/(1024**3))
        print(f"Method latency (seconds per {args.batch_size} images):", timing)
        print("Peak memory usage (MB):", memory)
        print("Total trainable parameters:", training_para_nums)