...
for steps in 2 3 4; do
    ...
    tag=$currenttime"_bs64_ToMe_"$tome_r"_best_params_steps_"${steps}
    ...
    for i in ${!corruptions[@]}; do
        ...
        echo $corruption
        python3 main_navia_multi_steps.py \
            --batch_size 64 \
            --workers 8 \
            --backbone $backbone \
            --ckpt_dir $ckpt_dir \
            ...
            --tag $tag --gradient_steps ${steps} > /home/xiongyizhe/CVPR2026/MGTTA/outputs/$algorithm"_"$tag/C/$corruption-running-log.txt
    done
done