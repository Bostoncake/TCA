...
tag=$currenttime"_bs64_ToMe_"$tome_r"_best_params_no_tome"
...
tag=$currenttime"_bs64_ToMe_"$tome_r"_best_params_no_tome_no_cls_tuning"
...
for i in ${!corruptions[@]}; do
    ...
    python3 main_navia_no_tome.py \
        ...
        --output ./outputs \
        --algorithm $algorithm \
        --apply_tome --tome_r $tome_r --apply_cls_ssf --cls_ssf_layer "-1" --as_baseline\
        --tag $tag > /home/xiongyizhe/CVPR2026/MGTTA/outputs/$algorithm"_"$tag/C/$corruption-running-log.txt
done