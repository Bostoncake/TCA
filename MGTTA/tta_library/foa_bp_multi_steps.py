...
class FOA_BP(nn.Module):
    ...
    def forward(self, x):
        ...
        outputs, batch_mean = forward_and_get_loss(x, self.model, self.optimizer, self.fitness_lambda, self.train_info, shift_vector, self.imagenet_mask, return_img_feat=return_img_feat, img_feat_stats=(self.ori_img_stds, self.ori_img_means), img_feat_lambda=self.img_feat_lambda, steps=self.steps)
...
def forward_and_get_loss(images, model:PromptViT, optimizer, fitness_lambda, train_info, shift_vector, imagenet_mask, return_img_feat=False, img_feat_stats=None, img_feat_lambda=1, steps=1):
