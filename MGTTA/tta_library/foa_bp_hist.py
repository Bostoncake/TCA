...
class FOA_BP_HIST(nn.Module):
    ...
    def _get_shift_vector(self):
        ...
        return self.hist_stat is not None
    
    def _init_shift_vector(self, init_vector):
        self.model.shift_vector.data.copy_(init_vector.detach())
    ...
    def forward(self, x):
        ...
        if not shift_vector:
            ...
            with torch.no_grad():
                self._init_shift_vector(-self.hist_stat)
        return outputs
    
    def obtain_origin_stat(self, train_loader, return_img_feat=False):
...
def forward_and_get_loss(images, model:PromptViT, optimizer, fitness_lambda, train_info, shift_vector, imagenet_mask, return_img_feat=False, img_feat_stats=None, img_feat_lambda=1):
    ...
    if shift_vector:
        output = model.vit.head(cls_features + 1. * model.shift_vector + 1. * train_info[1][-768:].detach())
        if imagenet_mask:
            output = output[:, imagenet_mask]
    ...
    # with torch.no_grad():
    #     if shift_vector:
    #         output = model.vit.head(cls_features + 1. * model.shift_vector)
    #         if imagenet_mask:
    #             output = output[:, imagenet_mask]
...
def collect_params_sep_cls_sep_ssf_sep_shift(model, lr_model, lr_cls, lr_ssf, lr_shift):
    """Collect the affine scale + shift parameters from batch norms.
    Walk the model's modules and collect all batch normalization parameters.
    Return the parameters and their names.
    Note: other choices of parameterization are possible!
