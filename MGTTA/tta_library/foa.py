...
class FOA(nn.Module):
    ...
    def _init_cma(self):
        ...
        dim = self.model.prompts.numel()        # 3 * embed_dim
        popsize = math.floor(4 + 3*math.log(dim)) # which is equal 27 when #prompts=3 and dim=768(base model)
        cma_opts = {
            'seed': 2020,
            'popsize': popsize,
            'maxiter': -1,
    ...
    def forward(self, x):
        ...
        prompts, losses = self.es.ask() + [self.best_prompts.flatten().cpu()], []#在CMA-ES中采样一定数量的prompt
        for j, prompt in enumerate(prompts):
            self.model.prompts = torch.nn.Parameter(torch.tensor(prompt, dtype=torch.float).
                                                        reshape_as(self.model.prompts).cuda()) #替换新的prompt
            self.model.prompts.requires_grad_(False)#不用计算梯度
            ...
            batch_means.append(batch_mean[-self.embed_dim:].unsqueeze(0))
            del batch_mean

            if self.best_loss > loss.item(): #记录loss最小的prompt对应的数据
        ...
        self.es.tell(prompts, losses)#根据prompt的rank来更新CMA-ES
        
        """Update overall test statistics, Eqn. (9)"""
        batch_means = torch.cat(batch_means, dim=0).mean(0)#更新测试样本CLS的统计量
        self._update_hist(batch_means)
        return self.best_outputs #返回最好的结果
...
def forward_and_get_loss(images, model:PromptViT, fitness_lambda, train_info, shift_vector, imagenet_mask):
    ...
    discrepancy_loss = fitness_lambda * (std_mse.sum() + mean_mse.sum())
    
    cls_features = features[:, -model.vit.embed_dim:] # the feature of classification token
