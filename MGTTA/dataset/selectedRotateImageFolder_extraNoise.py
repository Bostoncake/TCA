...
# normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) #for ResNet
...
common_corruptions_15 = ['gaussian_noise', 'shot_noise', 'impulse_noise', 'defocus_blur', 'glass_blur', 
                         'motion_blur', 'zoom_blur', 'snow', 'frost', 'fog', 'brightness', 
                         'contrast', 'elastic_transform', 'pixelate', 'jpeg_compression']

def load_list_from_pth(file_name):
...
class SelectedRotateImageFolder(datasets.ImageFolder):
    def __init__(self, root, train_transform, original=True, rotation=True, rotation_transform=None, shuffle=True):
...
def prepare_test_data_extra_noise(args, use_transforms=True, semi=False):
    if args.corruption == 'original' or args.corruption == 'rendition' or args.corruption == 'v2' or args.corruption == 'sketch' or args.corruption == 'imagenet_a':
        te_transforms_local = te_transforms if use_transforms else None
    elif args.corruption in common_corruptions or args.corruption == 'imagenet_c_val_mix':
        print("Use extra Gaussian noise")
        te_transforms_imageC_extra_noise = transforms.Compose([transforms.CenterCrop(224),
									transforms.ToTensor(),
                                    AddGaussianNoise(mean=0.0, std=args.gaussian_noise),
									normalize])
        te_transforms_local = te_transforms_imageC_extra_noise if use_transforms else None
    else:
    ...
    if args.corruption == 'original':
        ...
        teset = SelectedRotateImageFolder(validdir, te_transforms_local, original=False, rotation=False,
                                                    rotation_transform=rotation_te_transforms, shuffle=args.if_shuffle)
        ...
        teset = datasets.ImageFolder(validdir, te_transforms_local)
    elif args.corruption == 'imagenet_c_val_mix':
        dataset_list = []
        for args.corruption in ['speckle_noise', 'spatter', 'gaussian_blur', 'saturate']:
            validdir = os.path.join(args.data_corruption, args.corruption, str(args.level))
            teset = SelectedRotateImageFolder(validdir, te_transforms_local, original=False, rotation=False,
                                                        rotation_transform=rotation_te_transforms)
            teset.switch_mode(True, False)    
            dataset_list.append(teset)
        teset = torch.utils.data.ConcatDataset(dataset_list)#合并多个dataset
        if args.used_data_num != -1:
            # create subset
            subset_indices = random.sample(range(len(teset)), args.used_data_num)
            teset = Subset(teset, subset_indices)
        args.corruption == 'imagenet_c_val_mix'
    else:
    ...
    # print('len(teset):', len(teset))
    teloader = torch.utils.data.DataLoader(teset, batch_size=args.batch_size, shuffle=args.if_shuffle,  num_workers=args.workers, pin_memory=True)
