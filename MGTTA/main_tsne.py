...
import tta_library.foa_bp_tsne as foa_bp
...
def validate(val_loader, model, args, pretrain=False):
    ...
    with torch.no_grad():
        ...
        for i, dl in enumerate(val_loader):
            ...
            if i == args.start_validation:
                # re-initialize
                top1_val = AverageMeter('Acc@1', ':6.2f')
                top5_val = AverageMeter('Acc@1', ':6.2f')
            ...
            if args.algorithm == 'tent_bp_gtloss':
                if args.count_flops:
                    ...
                    if i < args.num_adapt_iters:
                        output = model(images, target)#需要gt算gtloss                
                    elif args.num_adapt_iters <= i < args.start_validation:
                        continue
                    else: 
                        output = model(images, target, update=False)#需要gt算gtloss                
            else:
                if args.count_flops:
                    ...
                    if i < args.num_adapt_iters:
                        ...
                        output, features = model(images, update=False)
                        features_list.append(features)
                        eval_targets_list.append(target)
            if not pretrain:
            ...
            if i >= args.start_validation:
                top1_val.update(acc1[0], images.size(0))
                top5_val.update(acc5[0], images.size(0))
    ...
    # save features before exit
    print("Save features for t-SNE")
    features_list = torch.cat(features_list, dim=0)
    eval_targets_list = torch.cat(eval_targets_list, dim=0)
    torch.save(features_list, os.path.join(args.output, "%s-cls_feats.pt"%(args.corruption)))
    torch.save(eval_targets_list, os.path.join(args.output, "%s-cls_tgts.pt"%(args.corruption)))
    exit(0)

    # return top1.avg, top5.avg, ece_avg
    return top1_val.avg, top5_val.avg
...
def get_args():
    ...
    parser.add_argument('--num_adapt_iters', default=75, type=int, help='number of iterations adapted before evaluation')
    parser.add_argument('--start_validation', default=312, type=int, help='index of the iteration that validation starts during convergence analysis')
...
if __name__ == '__main__':
    ...
    if args.quant:
        ...
        # checkpoint_path = "/nlp_group/xiongyizhe/research/TTA/foa_xyz/ckpt/vit-b.pth"
    ...
    for corrupt in corruptions:
        ...
        if args.algorithm == 'train_mgg':
            ...
            if not args.count_flops and not args.test_batch_time:
                top1_val, top5_val = validate(val_loader, adapt_model, args)
        ...
        logger.info(f"Under shift type {args.corruption} After {args.algorithm} Top-1 Convergence Accuracy: {top1_val:.6f} and Top-5 Convergence Accuracy: {top5_val:.6f}")
